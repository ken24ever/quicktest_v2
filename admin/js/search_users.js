...
<<<<<<< HEAD
    success: function (data) {
      ...
      // Update the completed exams count
      completedExamCount = data.completedCount;
     

      // Save the data in a variable and render the initial table
      var initialData = data;
      var dataSize = data.totalUsers;
...
<<<<<<< HEAD
  ...
  // Check if there is more than one user selected
  if (selectedUserCount > 1) {
    selectedUserName = selectedUserCount + ' users';
  } else {
    selectedUserName = $('.user-checkbox:checked').first().data('user-name'); // Use data-user-name consistently
  }
  ...
  Swal.fire({
    ...
    if (result.isConfirmed) {
      // User clicked "Yes, reset it!" button
      // Call the function to reset the selected user's exams
...
<<<<<<< HEAD
    ...
    if (selectedUserCount > 1) {
      ...
      selectedUserName = $('.user-checkbox:checked').first().data('user-name');
    }
    ...
    Swal.fire({
      ...
      if (result.isConfirmed) {
        ...
        // Call the function to delete the selected user(s)
