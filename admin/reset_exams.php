    ...
    if ($examResult && mysqli_num_rows($examResult) > 0) {
        ...
        if (mysqli_query($conn, $query)) {
            ...
            if (mysqli_query($conn, $truncateTable)) {
                // Update the remaining_time column in the users table
                $updateRemainingTimeQuery = "UPDATE users SET remaining_time = NULL WHERE id = '$userId'";
                mysqli_query($conn, $updateRemainingTimeQuery);
