...
var lastAttemptedQuestionKey = "lastAttemptedQuestion_" + userID;
// Load the last attempted question from localStorage
var lastAttemptedQuestion = parseInt(localStorage.getItem(lastAttemptedQuestionKey)) || 0;
...
function retrieveSelectedOptions(questions, callback) {
 ...
 // Add a variable to store the last attempted question
var lastAttemptedQuestion = 0;

// Add a function to persist the last attempted question
function persistLastAttemptedQuestion() {
  localStorage.setItem(lastAttemptedQuestionKey, lastAttemptedQuestion);
}

// Modify the loadQuestions function to check for the last attempted question
function loadQuestions(page) {
  // Check if there is a last attempted question stored in localStorage
  var storedLastAttemptedQuestion = localStorage.getItem(lastAttemptedQuestionKey);
  
  if (storedLastAttemptedQuestion !== null) {
    // If a last attempted question is found, use it as the starting point
    lastAttemptedQuestion = parseInt(storedLastAttemptedQuestion);
  } else {
    // Otherwise, set the default last attempted question to 0
  ...
  $.ajax({
    ...
    data: {
      ...
      page: lastAttemptedQuestion // Use the last attempted question as the starting point
    },
    dataType: 'json',
    success: function (response) {
      if (response.status === 'success') {
        ...
        // Update the current page based on the last attempted question
        currentPage = lastAttemptedQuestion;
        ...
        retrieveSelectedOptions(questions, function(updatedSelectedOptions) {
          // Merge the retrieved selected options with the existing ones
          selectedOptions = { ...selectedOptions, ...updatedSelectedOptions };
...
// Modify displayQuestion Function
  ...
  if (questions && questions.length > 0) {
    ...
    if (page === 0) {
      ...
      previousButton.click(function () {
        // Decrement the lastAttemptedQuestion and persist the change
        lastAttemptedQuestion = Math.max(0, lastAttemptedQuestion - 1);
        persistLastAttemptedQuestion();
    ...
    if (page === questions.length - 1) {
      ...
      nextButton.click(function () {
        // Increment the lastAttemptedQuestion and persist the change
        lastAttemptedQuestion = Math.min(questions.length - 1, lastAttemptedQuestion + 1);
        persistLastAttemptedQuestion();
