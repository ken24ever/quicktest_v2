  ...
  if (is_array($selectedOptions)) {
    ...
    foreach ($selectedOptions as $questionId => $options) {
      ...
      if ($result->num_rows > 0) {
        ...
        // Record the transaction in transactions_breakdown
        $recordTransactionSql = "INSERT INTO transactions_breakdown (exam_id, question_id, user_id) VALUES (?, ?, ?)";
        $recordTransactionStmt = $conn->prepare($recordTransactionSql);
        $recordTransactionStmt->bind_param('iii', $examId, $questionId, $usersExamId);
        $recordTransactionStmt->execute();
